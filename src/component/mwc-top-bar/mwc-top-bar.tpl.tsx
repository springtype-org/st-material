...
export default (component: MwcTopBar) => {
    ...
    if (component.showMenuButton) {
        section.push(
            <button class="material-icons mdc-top-app-bar__navigation-icon mdc-icon-button">{component.menuIcon}</button>
        );
    }
    section.push(<span class="mdc-top-app-bar__title">{component.title}</span>);
    return (
        <header id="app-bar" ref={{topBarRef: component}} class={appBarClasses}>
